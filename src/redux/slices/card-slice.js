...
export const cardSlice = createSlice({
    ...
    reducers: {
        ...
        subtocard: (state, action) => {
            ...
            if (existingItem) {
                state.value.carditems = state.value.carditems.map(item => 
                    item.id === newitem.id ? (item.quantity? { ...item, quantity: item.quantity - 1 } : { ...item, quantity: 0 }) : item
                );
            }
        },
...
export const { togglewindow, addtocard, removeitem, subtocard } = cardSlice.actions;
